...
- `20260518000002_professional_tier_description.sql` — trims the Professional tier `description` to a single audience line ("For full-time groomers and stylists working professionally in Maine."). The previous realignment migration listed every benefit in prose, which then rendered on `/join` immediately above the same items as a checkmark list. The benefits now live only in the checkmark area.
- `20260518000003_tier_perks.sql` — adds `perks text[] not null default '{}'` to `tiers` and seeds each existing tier's bullet list verbatim with what the Join page already rendered. The admin Settings → Tier configuration page now drives every bullet shown on `/join` via a repeatable list editor; the static `TIER_PRESENTATION.benefits` arrays on the Join page are gone. The `directory_listing` and `umbrella_account` columns stay as behavior flags (decoupled from display copy).
- `20260518000004_profile_address_and_salon_details.sql` — expands the member profile and salon (organization) shape so the dashboard Profile page and the public Join form can collect richer data in one pass. Adds `first_name`, `last_name`, `address_line`, `city`, `zip`, `state` (default `'ME'`) to `profiles`. Adds `address_line`, `city`, `zip`, `state` (default `'ME'`), `phone`, `website` to `organizations`. Backfills `first_name` / `last_name` by splitting any existing `full_name` on the first space. Rewrites `create_profile_on_signup` to copy `first_name` / `last_name` out of `raw_user_meta_data` (in addition to `full_name`). Adds an `organizations_owner_update` RLS policy so the salon's `primary_contact_profile_id` can edit their own org row from the dashboard. `full_name` stays as a column; the application keeps it in sync with `first_name + ' ' + last_name` on every write so all existing admin queries reading `full_name` keep working.
- `20260518000005_public_salons_rpc.sql` — adds two security-definer RPCs that back the new public `/directory` page. `public_salons(p_city, p_limit)` returns `(id, name, city, state, phone, website)` for salons whose primary contact holds an Active or Honorary membership, optionally filtered by city. `public_salon_cities()` returns the distinct sorted list of cities with at least one publishable salon, used for the filter dropdown. Both run security-definer so anon + authenticated can browse without lifting the admin-only RLS on `organizations`. Street address is intentionally not projected so sole props running out of a home address aren't exposed; phone + website stay because they're public business info.
